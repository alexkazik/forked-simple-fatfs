...
fn test_sfn_generator_long() {
    let mut gen = SfnGenerator::new("HELLO-WORLD.TXT", Codepage::default());
...
fn test_sfn_generator_short() {
    let mut gen = SfnGenerator::new("run.jpeg", Codepage::default());
...
fn test_sfn_generator_unknown_chars() {
    let mut gen = SfnGenerator::new("😇.😈", Codepage::default());
